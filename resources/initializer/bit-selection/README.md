...
- 预热逐题调的是 `/rag/v3/chat` 直答链路，而 `agent` 档下这条链路不执行任何 MCP 工具（这是故意设的闸门，见 `RetrievalEngine`）。业务数据类问法在预热里必然答不出来，放进来等于把闸门正常工作记成一条失败的演示数据

还有一条与商品数据相关的：**问法里的预算要落在真实价格带内**。在售手机最低是 5299 元，写「3000 以内买台手机」会得到一个空结果，而那不是检索的问题。

查订单、查物流、下单改单这些能力由对话页的 Agent 链路承担，那条链路走的是工具而不是检索。这也是本数据集与 `enterprise-knowledge-base` 的一处真实差异：后者跑在 `workflow` 档，MCP 类问法在它的预热里是走得通的。
